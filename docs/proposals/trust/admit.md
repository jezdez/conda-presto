...
The current {doc}`verification endpoint <../../reference/http-api>` checks a supplied artifact and bundle against the recipient's expected identity and issuer, with limited signing-step checks. It does not decide whether packages are suitable for an organization or whether installation should proceed.
