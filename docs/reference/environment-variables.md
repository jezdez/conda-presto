...
| `CONDA_PRESTO_SOLVER_CACHE_HOTSET_SIZE` | `32` | Maximum successful foreground solver workloads retained as local cache-warming candidates. Set to `0` to disable tracking. |
| `CONDA_PRESTO_SOLVER_CACHE_HOTSET_PERSIST` | `false` | Persist credential-free solver workload state in a configured file or Redis result store. Requires a persistent result-cache backend. |
